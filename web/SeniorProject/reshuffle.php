...
/*This service accepts the array of shuffled cards from the host player, and stores it in 
    the reshuffle table in the database
*/
...
//First, delete any previous decks from this game, if this is another deck restart
$stmt = $db->prepare('DELETE FROM public.reshuffle WHERE game_id = :game_id;');
$stmt->execute(array(':game_id' => $game_id));
$rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
//Insert each card in the deck into the public.reshuffle table
for ($i=0; $i<count($cardArray); $i++) {
    //Add each card to the database
    $stmt = $db->prepare('INSERT into public.reshuffle(game_id, card_id, position_in_deck) 
        VALUES (:game_id, :card_id, :position_in_deck);');
    $stmt->execute(array(':game_id' => $game_id, ':card_id' => $cardArray[$i]->id, ':position_in_deck' => $i));
}
//tell the update_manager that each player now needs to pull from the reshuffle table
...
for ($i=0; $i<count($playerRows); $i++) {
    $JSONstr = '{ "desc": "reshuffle", "info": ' . $game_id . '}';
